...
export default function OrdersAdmin({ orders }: { orders: Order[] }) {
  ...
  return (
    <ul className="flex flex-col divide-y divide-line border-t border-line">
      {items.map((order) => (
        <li key={order.id} className="flex flex-col gap-3 py-5 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0 flex-1">
            <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
              <span className="font-mono text-sm text-ink">#{order.id}</span>
              <span className="font-mono text-xs text-fog">{formatDate(order.createdAt)}</span>
            </div>
            ...
            {order.note && (
              <p className="mt-2 font-mono text-xs italic text-fog">"{order.note}"</p>
            )}
          </div>
